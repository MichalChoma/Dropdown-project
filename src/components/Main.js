  ...
  return (
    <main className='w-full lg:flex lg:items-center lg:justify-evenly z-0'>
        <div className='w-full mb-8 lg:w-[500px] lg:order-last'>
            <img src={heroImg} alt="hero img" className="w-full lg:hidden"/>
            <img src={heroDesktop} alt="hero img" className="object-cover hidden lg:block"/>
        </div>
        <div className='flex flex-col lg:w-6/12 '>
        <div className='flex flex-col items-center justify-center lg:-order-3 lg:items-start'>
            <h1 className='font-bold text-4xl my-8 lg:text-8xl z-0'>{mainContent.h1}</h1>
            <p className='font-medium text-gray-700 text-md text-center px-2 mb-8 lg:text-start lg:text-lg '>{mainContent.desc}</p>
            <Button>{mainContent.btn}</Button>
        </div>
